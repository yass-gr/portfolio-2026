          ...
          {isMobile ? (
            <div className="flex flex-col items-center gap-4 py-4">
              {buckets.map((bucket, bi) => (
                <div
                  key={bucket.label}
                  className="w-full aspect-square rounded-[60px] max-sm:rounded-[60px] max-lg:rounded-[60px] max-lg:max-w-lg max-lg:mx-auto relative bucket-card overflow-hidden"
                  style={{
                    border: `1.5px solid ${borderColor}`,
                    backgroundColor: blurBg,
                  }}
